...
If any required field (title, start, end) is missing or unclear, leave it blank.
  ...
  const listText = events.length
    ? events
        .map(e => {
          ...
          const start = startTime ? new Date(startTime).toLocaleString() : 'unknown time';
          const end = endTime ? new Date(endTime).toLocaleString() : 'unknown time';
          return `${e.id}: ${e.summary} from ${start} to ${end}`;
        })
        .join('\n')
...
Given the message below, identify which (if any) event to update and output a JSON object with these properties:
- eventId: string
- title: string
- location: string
- description: string
